...
class pos_commande(models.Model):
    _inherit = "pos.commande"
    
    bon_cmd = fields.One2many('pos.order', 'commande_principale', string="Bons de commandes", help="Ce champ contien la liste des BC reliées avec cette commande")
    ...
    @api.depends('bon_cmd','amount_total')
    def get_montants(self):
        for record in self:
            somme_montant_paye = 0
            for  pod_order_record in record.bon_cmd:
                somme_montant_paye += pod_order_record.amount_paid
